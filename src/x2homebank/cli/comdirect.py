...
from ..importer.comdirect import ComdirectImporter
from .display import display_csv


@click.group()
def comdirect2homebank():
    """
    A tool to process financial transaction CSV files in Comdirect format.
...
def show(input_filename: str, color: bool, sort_direction: str):
    ...
    display_csv(ComdirectImporter(), input_filename, color, date_ascending)
...
def main():
    comdirect2homebank.add_command(show)
    comdirect2homebank.add_command(convert)
    comdirect2homebank()
